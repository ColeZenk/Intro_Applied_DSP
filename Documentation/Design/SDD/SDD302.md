...
SW2 (PTC1) and SW3 (PTB17) are configured as GPIO inputs with internal pullups and falling edge interrupts (IRQC = 0xA). PORTB and PORTC interrupts are enabled via NVIC. SW2 increments K toward KMAX=10, SW3 decrements toward K=0 (DC only). Phase is reset to zero on each K change to avoid discontinuities in the output waveform.


## LED

The red LED (PTA1) is used to measure ISR execution time. PORTA clock is gated via SCGC5, PTA1 is configured as GPIO output via PCR MUX(1) and PDDR. The LED is active low — PCOR sets it on at ISR entry, PSOR sets it off at ISR exit. Measured pulse width on the oscilloscope was approximately 350ns, corresponding to ~0.35% of the 100μs sample period.  Processing time does not vary with K since the ISR performs only a LUT lookup regardless of the number of harmonics.
