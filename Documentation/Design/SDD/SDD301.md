...
>>> plt.show()
```
![Alt text](./Artifacts/SDD_301_Artifact_3.png "Figure 3")

As you can see, at high K y_k(t) is nearly identical asside from clipping and clipping margins.

## Final MATLAB Simulation

For the final simulation I chose MATLAB (as required). Mapping it from python to MATLAB is fairly trivial. This script runs a comparison of all k values (excluding DC).
```matlab
%% Fourier Series Synthesizer - MATLAB Validation
%  Mirrors fourier_synth.c LUT implementation
%  ECE 444 HW3 - Cole Zenk

%% Constants (match fourier_synth.h)
Fs             = 10000;
KMAX           = 10;
C_3            = 18000;
f0             = C_3/40;
...
%% Coefficients (match fourier_synth.c)
a_k = [1216.31250, 239.289971, -431.916129, -210.324415,  140.142399, ...
          8.48982269, -109.377703,  -0.805273241,  40.4644281, -13.1241907, -34.1638594];

b_k = [0, -165.95771119, -195.58403878,  23.32248174,  137.37739398, ...
       -55.14791232,  -97.56772502,  87.26825469,   45.95872345, -94.88022582, 4.83160338];

%% Build LUT (mirrors fourier_synth_init)
lut = zeros(KMAX + 1, PERIOD_SAMPLES);

for K = 0:KMAX
    for n = 0:PERIOD_SAMPLES-1
        t = n / Fs;
        y = a_k(1);                          % DC term, a_k(1) = k=0
        for k = 1:K
            y = y + 2*a_k(k+1)*cos(k*omega_0*t) ...
                  - 2*b_k(k+1)*sin(k*omega_0*t);
        end
        y = max(0, min(4095, y));
        lut(K+1, n+1) = y;                  % MATLAB 1-indexed
    end
end

%% Plot all K values (matches SDD301 Artifact 4)
figure('Position', [100 100 800 1000]);
for K = 1:KMAX
    subplot(5, 2, K);
    stairs(repmat(lut(K+1, :), 1, 2));      % 2 periods, step plot
    title(['K=' num2str(K)]);
    ylim([0 4095]);
    grid on;
end
sgtitle('y_K(t) - LUT Output for K=1 to 10');
```

![Alt text](./Artifacts/SDD_301_Artifact_4.png "Figure 4")
