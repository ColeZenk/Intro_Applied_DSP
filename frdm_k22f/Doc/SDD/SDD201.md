...
1. This seemingly minimal algorithm is that it can be perceived as a cosine (turns out Fourier was right on the money. Though this is a bit more direct than a transform). This is made obvious by observing the behavior of a normalized cosine. A cosine function will reach a maximum of 1 and a minimum of -1 at periodically. The condition is that the maximum of 1 must be observed at n = 0. So in other words, this function can be seen as a discrete cosine function (with a very minimally scoped bit mask or unfortunately chosen polling frequency applied). Furthermore, it can be deduced that this cosine is specifically equivalent to
...
**Implementation:** Simple LUT with modulo-4 counter.
