...
/* Fixed: Early WDOG disable, correct FCF, proper .size placement */
...
/* MUST be at address 0x400 - linker script places this */
...
flashconfig:
    ...
    .long   0xFFFFFFFF      /* 0x408: FPROT0-3 (all unprotected) */
    .byte   0xFE            /* 0x40C: FSEC = 0xFE (unsecured, backdoor enabled) */
    .byte   0xFF            /* 0x40D: FOPT = 0xFF (LPBOOT=1, NMI enabled, EzPort enabled, etc) */
...
Reset_Handler:
    ...
    /* Must unlock, then disable within 256 bus clocks */
    ldr r0, =0x40052000         /* WDOG base address */
    
    /* Unlock sequence: write 0xC520 then 0xD928 to UNLOCK (offset 0x0E) */
    ldr r1, =0xC520
    strh r1, [r0, #0x0E]        /* WDOG_UNLOCK = 0xC520 */
    ldr r1, =0xD928
    strh r1, [r0, #0x0E]        /* WDOG_UNLOCK = 0xD928 */
    
    /* Disable WDOG: clear WDOGEN bit in STCTRLH (offset 0x00) */
    /* STCTRLH = 0x01D2 disables watchdog (WDOGEN=0) */
    ldr r1, =0x01D2
    strh r1, [r0, #0x00]        /* WDOG_STCTRLH */
    /* ===== End watchdog disable ===== */

    /* Set stack pointer (in case we got here without reset) */
    ldr sp, =_estack
